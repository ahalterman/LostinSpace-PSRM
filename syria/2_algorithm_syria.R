...
set.seed(2015)
sampled1<-sample(1:3, nrow(syria_data), replace=TRUE)
set.seed(2016)
...
# save the predicted PROBABILITIES for graphics
for ( i in 1:k) {
  dat<-paste0("seed_",(2014+i) )
  fram<-as.data.frame(cbind( as.character(s.id[[i]]),
...
syria_all<-buildY(texts=syria_data$cleantext, namespace=names,
                  solution=syria_data$province_human, text_no=syria_data$story_id)
syria_all$id<-paste0(syria_all$text_id, syria_all$location_names)
...
for( i in 1:length(id)){
  y[i]<-as.numeric(as.character(syria_all$Y[which(id[i]==syria_all$id)] ) )
}
syria_results.all<-as.data.frame(cbind(id, "original"=y,
...
write.csv(syria_results.all, "syria_results.all.csv")



############## End of syria_final.R ###############
